...
public class BeatAnalyzer {
    ...
    public AnalyzeBeatResult AnalyzeBeat(int[] beat)
    {
        ...
        // Search forward from the R-wave to find an isoelectric region following the QRS complex.
        ...
        if(maxSlopeI < minSlopeI)
        {
            ...
            for(i = minSlopeI; (i < bdacParas.BEATLGTH) && ((beat[i] - beat[i-1]) < (minSlope >>2)); ++i);
                result.offset = i ;
            ...
            for(i = maxSlopeI;(i < bdacParas.BEATLGTH) && ((beat[i] - beat[i-1]) > (maxSlope >>2)); ++i);
                result.offset = i ;
